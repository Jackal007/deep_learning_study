...
datasets = datasets.reshape(-1, window_size, 9, 9, 1)
one_hot_labels = np.array(list(pd.get_dummies(labels)))  # some thing wrong
###############################################################################
...
datasets_test = datasets.reshape(-1, window_size, 9, 9, 1)
one_hot_labels = np.array(
    list(pd.get_dummies(labels_test)))  # some thing wrong
###############################################################################
print(one_hot_labels.shape)
labels_test = np.asarray(pd.get_dummies(labels_test), dtype=np.int8)
test_x = datasets_test
test_y = labels_test
...
print("**********("+time.asctime(time.localtime(time.time())) +
      ") Load and Split dataset End **********\n")

print("**********("+time.asctime(time.localtime(time.time())) +
      ") Define parameters and functions Begin: **********\n")
...
print("**********("+time.asctime(time.localtime(time.time())) +
      ") Define parameters and functions End **********\n")

print("**********("+time.asctime(time.localtime(time.time())) +
      ") Define NN structure Begin: **********\n")
...
print("**********("+time.asctime(time.localtime(time.time())) +
      ") Define NN structure End **********\n")

print("**********("+time.asctime(time.localtime(time.time())) +
      ") Train and Test NN Begin: **********\n")
# run
...
with tf.Session(config=config) as session:
    ...
    for epoch in range(training_epochs):
        ...
        for b in range(batch_num_per_epoch):
            ...
            batch_x = batch_x.reshape(len(batch_x)*(int(window_size)), 9, 9, 1)
        ...
        if(epoch % 1 == 0):
            ...
            for i in range(train_accuracy_batch_num):
                ...
                train_batch_x = train_batch_x.reshape(
                    len(train_batch_x)*window_size, 9, 9, 1)
            ...
            for j in range(test_accuracy_batch_num):
                ...
                test_batch_x = test_batch_x.reshape(
                    len(test_batch_x)*window_size, 9, 9, 1)
    ...
    print("("+time.asctime(time.localtime(time.time()))+") Final Test Cost: ",
          np.mean(test_loss), "Final Test Accuracy: ", np.mean(test_accuracy))
    # save result
