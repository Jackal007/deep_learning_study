...
def RNN(X):
    ...
    weights = {
        ...
        'output': tf.Variable(tf.random_normal([fc5_nuits, n_classes]), trainable=True),
    }
    biases = {
    ...
    fc2_layer = tf.nn.elu(
        tf.add(
            tf.matmul(fc1_layer, weights['fc1']),
            biases['fc1']))

    fc3_layer = tf.nn.tanh(
        tf.add(
            tf.matmul(fc2_layer, weights['fc1']),
            biases['fc1']))
    fc4_layer = tf.nn.sigmod(
        tf.add(
            tf.matmul(fc3_layer, weights['fc1']),
            biases['fc1']))
    fc5_layer = tf.nn.relu(
        tf.add(
            tf.matmul(fc4_layer, weights['fc1']),
            biases['fc1']))

    results = tf.nn.relu(tf.matmul(fc5_layer, weights['output']) +
                         biases['output'])
